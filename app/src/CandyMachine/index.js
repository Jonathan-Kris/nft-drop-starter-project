...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getCandyMachineState = async () => {
    ...
    // Fetch Candy Machine Instance
    const candyMachine = await program.account.candyMachine.fetch(
      process.env.REACT_APP_CANDY_MACHINE_ID
    );
    // Get Metadata
    const itemsAvailable = candyMachine.data.itemAvailable.toNumber();
    const itemsRedeemed = candyMachine.data.itemRedeemed.toNumber();
    const itemsRemaining = itemsAvailable - itemsRedeemed;
    const goLiveDate = candyMachine.data.goLiveData.toNumber();
    const presale =
